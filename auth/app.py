"""
Backend de acceso a /docs/ para la landing de Contalibra.
...
se pide al backoffice (admin.contalibra.com.ar/api/clientes-publicos).
...
SECRET_KEY       = os.environ.get("SECRET_KEY", "contalibra-docs-secret-change-me")
ADMIN_API_URL    = os.environ.get("ADMIN_API_URL", "https://admin.contalibra.com.ar")

COOKIE_NAME  = "docs_session"
MAX_AGE      = 86400 * 7  # 7 días, igual que la sesión de la app
DOMAIN_RE    = re.compile(r"^[a-z0-9-]+\.contalibra\.com\.ar$")

_signer = URLSafeTimedSerializer(SECRET_KEY)

app = FastAPI(title="Contalibra Docs Auth", docs_url=None, redoc_url=None)
...
<head>
<meta charset="UTF-8">
<title>Acceso a Documentación · Contalibra</title>
...
<style>
  ...
  select, input {{ width:100%; padding:.6rem .75rem; border:1px solid #e2e8f0; border-radius:8px;
                   font-size:1rem; box-sizing:border-box; }}
  button {{ width:100%; margin-top:1.25rem; padding:.7rem; background:#2563eb; color:#fff;
...
<body>
  <div class="card">
    ...
    <form method="post" action="/login-docs">
      <label for="slug">Tu empresa</label>
...
async def login_submit(slug: str = Form(...), username: str = Form(...), password: str = Form(...)):
    ...
    if not cliente:
        return HTMLResponse(_render_login("Empresa no encontrada. Volvé a intentarlo."), status_code=400)

    domain = cliente.get("domain", "")
    if not DOMAIN_RE.match(domain):
        return HTMLResponse(_render_login("Configuración de empresa inválida."), status_code=400)
